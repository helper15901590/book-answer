...
if [ -n "${ADMIN_PHONE:-}" ] && [ -n "${ADMIN_PASSWORD:-}" ]; then
  LOGIN=$(curl -s -c "$JAR" -b "$JAR" -X POST "$BASE/api/admin/login" -H "Origin: $BASE" -H 'Content-Type: application/json' -d "{\"phone\":\"$ADMIN_PHONE\",\"password\":\"$ADMIN_PASSWORD\",\"secondPassword\":\"${ADMIN_SECOND_PASSWORD:-}\"}")
  ...
  if [ -z "$CSRF" ]; then
    ...
    if [ -n "$NEW_USER_ID" ]; then
      curl -s -b "$JAR" -X DELETE "$BASE/api/admin/users/$NEW_USER_ID" -H "Origin: $BASE" -H "X-CSRF-Token: $CSRF" >/dev/null
