...
`NODE_ENV`（production 启用生产静态资源模式）、`PORT`、`JWT_SECRET`（必填 ≥16 字符，无默认值，缺失拒绝启动）、`ADMIN_PHONE`/`ADMIN_PASSWORD`（管理后台登录凭证，账号不入库）、`DATA_DIR`、`TRUST_PROXY`、`HOST_PORT`（仅 compose 插值）、`DEEPSEEK_API_KEY`/`DEEPSEEK_BASE_URL`/`DEEPSEEK_MODEL`（OpenAI 兼容 LLM 配置，DeepSeek 或阿里 DashScope 二选一，见 .env.example）
...
| `src/admin-main.tsx` + `leonchan1590.html` | 管理后台独立多页入口 `/leonchan1590`（AdminGate 验证后渲染 AdminPanel） |
...
| `docs/` | `ONBOARDING.md` 架构导览、`deploy.md` 部署/备份/恢复手册 |
...
- 配额按会员等级（guest/free_member/monthly_member/quarterly_member/yearly_member）在 `checkAndConsumeQuota` 中控制
